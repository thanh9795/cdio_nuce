...
class Vanban_dinhkem extends CI_Controller {
	...
	{
		if ($id!=NULL) {
			$data=$this->Vanban_dinhkem_model->get_all('','id,type,link,date_created',['id_vanban'=>$id]);
		...
		if ($this->input->server('REQUEST_METHOD')=="POST") {
			...
			if ($type==1) {
				if ($this->input->post('link')!=NULL) {
					$this->Vanban_dinhkem_model->insert(['type'=>$type,'link'=>$this->input->post('link'),'id_vanban'=>$this->input->post('id_vanban')]);
				...
				$config['upload_path']          = './assets/vanban';
				...
				else
				{
					...
					$path="assets/vanban/".$this->upload->data()['file_name'];
					$this->Vanban_dinhkem_model->insert(['type'=>$type,'link'=>$path,'id_vanban'=>$this->input->post('id_vanban')]);
					echo json_encode(['code'=>'success','message'=>"Thêm mới đính kèm thành công"]);
				}
			}
		}
